...
namespace alias {
    ...
    // Return a 'n_draws' x 'dim' matrix simulated from quasi-random uniform distribution
    ArrayXXd quasi_random_uniform(long dim, long n_draws);

    // Convert a uniform matrix to normal draws (same shape as input matrix M).
    ArrayXXd quasi_random_uniform_to_normal(const Eigen::Ref<const ArrayXXd>& M, double mean, double stderr);
}
