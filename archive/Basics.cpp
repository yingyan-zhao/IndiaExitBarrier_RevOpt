...
ParaEst alias::constructParaEst(const ArrayXd & theta,const int & GoodState, const int & LaborIntensive) {
    ...
    if (GoodState == 1 & LaborIntensive == 1) {
        ...
        para_est.alpha_M = para.alpha_M_G_L;
        ...
        para_est.alpha_M = para.alpha_M_B_L;
        ...
        para_est.alpha_M = para.alpha_M_G_K;
        ...
        para_est.alpha_M = para.alpha_M_B_K;
    ...
    int nn = 1;
    para_est.H_K_Cons = 0.0;
    para_est.c_HK = theta(nn+0);
    para_est.H_K = 0.0;
    para_est.F_K_Cons = 0.0;
    para_est.c_FK = theta(nn+1);
    para_est.F_K = 0.0;

    para_est.H_ur_Cons = 0.0;
    para_est.c_H_ur = theta(nn+2);
    para_est.H_ur = 0.0;

    if (GoodState == 1) {
        para_est.F_high_ur_Cons = 0.0;
        para_est.c_high_F_ur = theta(nn+3);
        para_est.F_high_ur = 0.0;

        para_est.F_low_ur_Cons = 0.0;
        para_est.c_low_F_ur = theta(nn+4);
        para_est.F_low_ur = 0.0;
    }
    else {
        para_est.F_high_ur_Cons = 0.0;
        para_est.c_high_F_ur = theta(nn+5);
        para_est.F_high_ur = 0.0;

        para_est.F_low_ur_Cons = 0.0;
        para_est.c_low_F_ur = theta(nn+6);
        para_est.F_low_ur = 0.0;

    }

    para_est.H_uc_Cons = 0.0;
    para_est.c_H_uc = theta(nn+7);
    para_est.H_uc = 0.0;

    if (GoodState == 1) {
        para_est.F_uc_Cons = 0.0;
        para_est.c_F_uc = theta(nn+8);
        para_est.F_uc = 0.0;
    }
    else {
        para_est.F_uc_Cons = 0.0;
        para_est.c_F_uc = theta(nn+9);
        para_est.F_uc = 0.0;
    }

    para_est.sigma_Kerror = theta(nn+10);
    para_est.sigma_Lerror_ur = theta(nn+11);
    para_est.sigma_Lerror_uc = theta(nn+12);

    if (GoodState == 1) {
        para_est.F_E = theta(nn+13);
        para_est.w_ur = para.w_ur_G_L;
        para_est.w_uc = para.w_uc_G_L;
    }
    else if (GoodState == 0) {
        para_est.F_E = theta(nn+14);
        para_est.w_ur = para.w_ur_B_L;
        para_est.w_uc = para.w_uc_B_L;
    }

    para_est.F_E_c_FK = theta(0);
    para_est.F_E_c_F_ur = 0.0;
    para_est.F_E_c_F_uc = 0.0;
    //    if (GoodState == 1) {
    //        para_est.F_E_c_F_ur = theta(1);
    //        para_est.F_E_c_F_uc = theta(2);
    //    }
    //    else {
    //        para_est.F_E_c_F_ur = theta(1);
    //        para_est.F_E_c_F_uc = theta(2);
    ...
    para_est.sigma_SE = theta(nn+15);
    para_est.r_K = 0.0;

    // cout << "para_est.c_HK = " << para_est.c_HK << "; para_est.c_FK = " << para_est.c_FK
    //     << "; para_est.c_H_ur = " << para_est.c_H_ur << "; para_est.c_high_F_ur = " << para_est.c_high_F_ur << "; para_est.c_low_F_ur = " << para_est.c_low_F_ur
    //     << "; para_est.c_H_uc = " << para_est.c_H_uc << "; para_est.c_F_uc = " << para_est.c_F_uc
    //     << "; para_est.sigma_Kerror = " << para_est.sigma_Kerror << "; para_est.sigma_Lerror_ur = " << para_est.sigma_Lerror_ur
    //     << "; para_est.sigma_Lerror_uc = " << para_est.sigma_Lerror_uc
...
**************************************************************/
//// Construct the grid for the value function and policy functions
ParaVec alias::constructParaFull(const ParaEst & para_est, const double & p_K) {
    ParaVec para_vec;
    ...
    para_vec.vec_lnphi_cut = para.vec_norm_phi_cut*para_vec.prod_sigma0 + para_vec.prod_mu0*ArrayXd::Ones(para.N_phi-1);
    para_vec.vec_phi = exp(para_vec.vec_lnphi);
    para_vec.vec_phi_Full = KroneckerProduct(para_vec.vec_phi,ArrayXd::Ones(para.N_K*para.N_Lur*para.N_Luc));
    para_vec.vec_phi_Full_PD = ArrayXd::Zero(para.N_phi * para.N_K * para.N_Lur * para.N_Luc * 2);
    para_vec.vec_phi_Full_PD << para_vec.vec_phi_Full,para_vec.vec_phi_Full;

    // initial distribution
    para_vec.dist0 = ArrayXd::Zero(para.N_phi);
    ArrayXd dist0_temp = normCDF_vec(para_vec.vec_lnphi_cut, para_vec.prod_mu0, para_vec.prod_sigma0);
    para_vec.dist0(0) = dist0_temp(0);
    para_vec.dist0.segment(1,para.N_phi-2) = dist0_temp.segment(1,para.N_phi-2)
                                             - dist0_temp.segment(0,para.N_phi-2);
    para_vec.dist0(para.N_phi-1) = 1.0 - dist0_temp(para.N_phi-2);

//    cout << "para_est.prod_mu0 = " << para_est.prod_mu0 << "; para_est.prod_sigma0 = " << para_est.prod_sigma0 << endl;
//    cout << "para_vec.dist0 = " << para_vec.dist0.transpose() << endl;

    //// transition of theta
    ArrayXd mu_lnphi_vec = para_est.gamma1 * para_vec.vec_lnphi + para_est.gamma0;
    ArrayXXd tran_lnphi_CDF = normCDF_mat(para_vec.vec_lnphi_cut, mu_lnphi_vec, para_est.sigma_phi_eps);
    para_vec.tran_phi = ArrayXXd::Zero(para.N_phi,para.N_phi);
    para_vec.tran_phi.col(0) = tran_lnphi_CDF.col(0);
    para_vec.tran_phi(all,seqN(1,para.N_phi-2))
            = tran_lnphi_CDF(all,seqN(1,para.N_phi-2))
              - tran_lnphi_CDF(all,seqN(0,para.N_phi-2));
    para_vec.tran_phi.col(para.N_phi-1) = ArrayXd::Ones(para.N_phi) - tran_lnphi_CDF.col(para.N_phi-2);

//    cout << "para_vec.tran_phi = \n" << para_vec.tran_phi << endl;

    //// grids of Luc
    tuple<ArrayXd,ArrayXd,ArrayXd> t = genGridStates(para_est,para_vec.vec_phi);
    para_vec.vec_K =  get<0>(t);
    para_vec.vec_Lur =  get<1>(t);
    para_vec.vec_Luc =  get<2>(t);
//    cout << "para_vec.vec_Lur = " << para_vec.vec_Lur.transpose() << endl;

    ArrayXd temp_K = KroneckerProduct(para_vec.vec_K,ArrayXd::Ones(para.N_Luc*para.N_Lur));
    para_vec.vec_K_Full = KroneckerProduct(ArrayXd::Ones(para.N_phi),temp_K);
    para_vec.vec_K_Full_PD = ArrayXd::Zero(para.N_phi * para.N_K * para.N_Lur * para.N_Luc * 2);
    para_vec.vec_K_Full_PD << para_vec.vec_K_Full,para_vec.vec_K_Full;

    ArrayXd temp_Lur = KroneckerProduct(para_vec.vec_Lur,ArrayXd::Ones(para.N_Luc));
    para_vec.vec_Lur_Full = KroneckerProduct(ArrayXd::Ones(para.N_phi*para.N_K),temp_Lur);
    para_vec.vec_Lur_Full_PD = ArrayXd::Zero(para.N_phi * para.N_K * para.N_Lur * para.N_Luc * 2);
    para_vec.vec_Lur_Full_PD << para_vec.vec_Lur_Full,para_vec.vec_Lur_Full;

    para_vec.vec_Luc_Full = KroneckerProduct(ArrayXd::Ones(para.N_phi*para.N_K*para.N_Lur),para_vec.vec_Luc);
    para_vec.vec_Luc_Full_PD = ArrayXd::Zero(para.N_phi * para.N_K * para.N_Lur * para.N_Luc * 2);
    para_vec.vec_Luc_Full_PD << para_vec.vec_Luc_Full,para_vec.vec_Luc_Full;

    return para_vec;
}

//// get the Grids of L_uc and L_ur
tuple<ArrayXd,ArrayXd,ArrayXd> alias::genGridStates(const ParaEst & para_est, const ArrayXd & vec_phi) {

//    ArrayXd K_temp = ArrayXd::LinSpaced(para.N_phi+para.N_plus,0.001,3500);
//    ArrayXd L_ur_temp = ArrayXd::LinSpaced(para.N_phi+para.N_plus,0.00000001,150);
//    ArrayXd L_uc_temp = ArrayXd::LinSpaced(para.N_phi+para.N_plus,0.00000001,150);

    ArrayXd K_temp = exp( ArrayXd::LinSpaced(para.N_phi+para.N_plus,log(0.001),log(3500)));
    ArrayXd L_ur_temp = exp( ArrayXd::LinSpaced(para.N_phi+para.N_plus,log(1),log(1500)));
    ArrayXd L_uc_temp = exp( ArrayXd::LinSpaced(para.N_phi+para.N_plus,log(1),log(900)));

//    ArrayXd K_temp = exp( ArrayXd::LinSpaced(para.N_phi+para.N_plus,log(0.001),log(3500)));
//    ArrayXd L_ur_temp = exp( ArrayXd::LinSpaced(para.N_phi+para.N_plus,log(0.00000001),log(150)));
//    ArrayXd L_uc_temp = exp( ArrayXd::LinSpaced(para.N_phi+para.N_plus,log(0.00000001),log(150)));

    //// grids of Luc
    ArrayXi vec_Luc_temp_min = find_matlab((L_uc_temp < para.Lur_cutoff1).cast<int>());
    ArrayXi vec_Luc_temp_max = find_matlab((L_uc_temp > para.Lur_cutoff1).cast<int>());
    ArrayXd L_uc = ArrayXd::Zero(para.N_Luc);
    if (vec_Luc_temp_max.size() == 0) {
        L_uc << L_uc_temp, para.Lur_cutoff1;
    }
    else {
        L_uc << L_uc_temp(vec_Luc_temp_min), para.Lur_cutoff1, L_uc_temp(vec_Luc_temp_max);
    }
//    cout << "L_uc = " << L_uc.transpose() << endl;
//    throw runtime_error("292");

    //// grids of Lur
    ArrayXi vec_Lur_temp_min = find_matlab((L_ur_temp < para.Lur_cutoff1).cast<int>());
    ArrayXi vec_Lur_temp_max = find_matlab((L_ur_temp > para.Lur_cutoff1).cast<int>());
    ArrayXd L_ur = ArrayXd::Zero(para.N_Lur);
    if (vec_Lur_temp_max.size() == 0) {
        L_ur << L_ur_temp, para.Lur_cutoff1;
    }
    else {
        L_ur << L_ur_temp(vec_Lur_temp_min), para.Lur_cutoff1, L_ur_temp(vec_Lur_temp_max);
    }
//    cout << "L_ur = " << L_ur.transpose() << endl;
//    throw runtime_error("292");

    //// grids of K
    ArrayXd K = ArrayXd::Zero(para.N_K);
    K << K_temp;
//    throw runtime_error("292");
//    ArrayXd L_uc = ArrayXd::LinSpaced(para.N_Luc,0.1,1000);
//    ArrayXd L_ur = ArrayXd::LinSpaced(para.N_Lur,0.1,1000);
//    ArrayXd K = ArrayXd::LinSpaced(para.N_K,0.1,1000);

    return tuple<ArrayXd,ArrayXd,ArrayXd>(K,L_ur,L_uc);
}
