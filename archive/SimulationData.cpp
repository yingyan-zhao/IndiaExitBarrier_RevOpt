...
SimVar alias::SimulationBase(const int & SimN, const int & SimTbar) {
    ...
    const ArrayXXd randomdraw = quasi_random_uniform(dim, SimN); // SimN x (9T+1)

    sim_var.lnphi_sim = quasi_random_uniform_to_normal(randomdraw.leftCols(T + 1), 0.0, 1.0);
    sim_var.lnLur_sim = quasi_random_uniform_to_normal(randomdraw.middleCols(T + 1, T), 0.0, 1.0);

    sim_var.lnF_PD            = randomdraw.middleCols(2 * T + 1, T);
    sim_var.lnF_SE            = randomdraw.middleCols(3 * T + 1, T);
    sim_var.missing_sim       = randomdraw.middleCols(4 * T + 1, T);
    sim_var.PDsim_Empirical   = randomdraw.middleCols(5 * T + 1, T);
    sim_var.SEsim_Empirical   = randomdraw.middleCols(6 * T + 1, T);
    sim_var.Misssim_Empirical = randomdraw.middleCols(7 * T + 1, T);
    sim_var.StateCategory_Sim = randomdraw.middleCols(8 * T + 1, T);
