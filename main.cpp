...
int main() {
    ...
    **************************************************************/
    ...
    ArrayXd theta(para.dim1+para.dim2+para.dim3);
    theta << 103.318, 218.532, 178.482, 256.496, 200.002, 0.381529, -413.768,-626.425, 0.5, 528.122,
        0.198573, 0.180748, 0.892402, 0.713,
        -12.5651, 11.973, 9.1589;
    int GoodState = 1;
    int LaborIntensive = 1;
    ParaEst para_est_G_L = constructParaEst(theta, GoodState, LaborIntensive);
    para_est_G_L.PI = 1.0;
    ParaVec para_vec_G_L = constructParaFull(para_est_G_L);

    //// solve the value function policy fucntion
    ArrayXd VEnd_PD = ArrayXd::Zero(para.N_phi * para.N_K * para.N_Lur * 2);
    tuple<EquStateV,EquStateVmat> t = solveV(para_est_G_L,para_vec_G_L,VEnd_PD,threadsManagement);
...
//     theta_Est_S << 146.883, 237.402, 1663.17, 94.7867, 222.251, 171.607, 257.683, 210.736, 29.443, 36.1752, 34.7117,
//             0.376126, 0.376841, 0.680865, -402.643, -631.222, 528.497, 0.188998, 0.183877, 0.89393, 0.714784,
//             -21.0738, 20.5529, 15.4135;
////
//     /**************************************************************************************************************************/
...
//         t_StatusQuo = main_SimulateStatusQuo(theta_Est_S, sim_var, SimTbar, threadsManagement);
